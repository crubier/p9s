...
import { computeIndexRange, computeProduct, enumerateTreeNodesAndEdgesBreadthFirst, generateRandomBitmap, generateRandomPairsInRanges, generateRegularPairsInRanges, generateUuidFromInteger, type PostgresTestContext } from './testing.mts';
import { setupTests } from './testing.mts';
import { NIL as NIL_UUID } from 'uuid';
setupTests();

test<PostgresTestContext>('SQL uuid benchmark', async ({ client, database_admin_username, database_user_username, runTestQuery }) => {
  ...
  await client.query(compile(sql`
    ...
    return sql`(${literal(generateUuidFromInteger(node.id))}::uuid)`
  }), ",")};
  `));
  ...
  ${join(resourceEdges.map((edge) => {
    return sql`(${literal(generateUuidFromInteger(edge.parent))}::uuid, ${literal(generateUuidFromInteger(edge.child))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.8, -edge.parent))}::bit(${literal(bitmapSize)}))`
  }), ",\n")};
`));
  ...
  await client.query(compile(sql`
    ...
    [
      ...
      ...assignmentRegularOrgOrgPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.3, index * 27367181))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRegularTeamWorkspacePairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.5, index * 8374))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRegularUserFolderPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.7, index * 15553))}::bit(${literal(bitmapSize)}))`
      }),

      ...assignmentRandomOrgOrgPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.3, index * 43627829))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomOrgWorkspacePairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.4, index * 828734))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomOrgFolderPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.4, index * 22651))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomOrgProjectPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.3, index * 127647))}::bit(${literal(bitmapSize)}))`
      }),

      ...assignmentRandomTeamOrgPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.3, index * 987394))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomTeamWorkspacePairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.4, index * 42123))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomTeamFolderPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.4, index * 212131))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomTeamProjectPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.3, index * 556455))}::bit(${literal(bitmapSize)}))`
      }),

      ...assignmentRandomUserOrgPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.5, index * 8787867))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomUserWorkspacePairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.4, index * 7373737))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomUserFolderPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.4, index * 3472497))}::bit(${literal(bitmapSize)}))`
      }),
      ...assignmentRandomUserProjectPairs.map(({ left, right }, index) => {
        return sql`(${literal(generateUuidFromInteger(left))}::uuid, ${literal(generateUuidFromInteger(right))}::uuid, b${literal(generateRandomBitmap(bitmapSize, 0.4, index * 4294797))}::bit(${literal(bitmapSize)}))`
      }),
    ]
